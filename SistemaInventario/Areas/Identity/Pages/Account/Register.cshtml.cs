        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    /*
                     * Verificamos la primera vez que se realiza el registro del usuario si existe alguno de los roles
                     * Si no existe el sistema crea un rol
                    **/
                    if (!await _roleManager.RoleExistsAsync(DS.Role_Admin))
                    {/*
                      * En este caso verificamos si existe un rol llamado Admin usando la clase DS
                      */
                        await _roleManager.CreateAsync(new IdentityRole(DS.Role_Admin));
                    }
                    if (!await _roleManager.RoleExistsAsync(DS.Role_Cliente))
                    {/*
                      * En este caso verificamos si existe un rol llamado Cliente usando la clase DS
                      */
                        await _roleManager.CreateAsync(new IdentityRole(DS.Role_Cliente));
                    }
                    if (!await _roleManager.RoleExistsAsync(DS.Role_Inventario))
                    {/*
                      * En este caso verificamos si existe un rol llamado Inventario usando la clase DS
                      */
                        await _roleManager.CreateAsync(new IdentityRole(DS.Role_Inventario));
                    }

                    /*Utilizamos el userManager para que en el formulario se agrege el rol al usuario
                     * El valor es el que se recibe desde el Page
                     * Validamos si el rol es null es por que es un nuevo usuario tipo cliente
                     */
                    if (user.Role == null)
                    {
                    ...
                    {
                        /**
                         * Cuando sea diferente de null es por que un usuario admin dio de alta el usuario
                         * Se asignará el rol que se haya seleccionado en el formulario
                         */
                    ...
                    {
                        /**
                         * Se deberá de realizar otra validación en la que verificamos si el usuario que se acaba de crear
                         * no tenia un rol entonces es un usuario cliente y deberá de ser redirigido y logueado
                         */
                        if (user.Role == null)
                        {
